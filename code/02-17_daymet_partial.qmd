...
```{r fiber strength}
ggplot(field, aes(x = strength_gtex)) +
  geom_density()
```
...
Daymet provides an application programing interface (API) for users to make queries and download data.  it is protocol for programming language to use data directly from website without visitng the website. lets say from r without visitng that website
