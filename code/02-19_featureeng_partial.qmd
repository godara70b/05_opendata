...
fe_month_sum_wide <- fe_month_sum %>%
  ...
  pivot_wider(
    id_cols    = c(year, site, strength_gtex),
    names_from = month_abb,
    values_from = mean_dayl.s:sum_prcp.mm,
    names_glue = "{month_abb}_{.value}",
    values_fn  = mean   # protects you if duplicates exist
  ) %>%
...
geNow let's do that for all variables.  
```{r}
library(ggridges)
library(ggplot2)
library(tidyverse)
...
finalplots <- fe_month_sum %>%
  pivot_longer(
    cols = mean_dayl.s:sum_prcp.mm,
    names_to = "name",
    values_to = "value"
  ) %>%
  mutate(
    month_abb = factor(month_abb, levels = month.abb)  # important
  ) %>%
  group_by(name) %>%
  nest() %>%
  mutate(
    plot = map2(data, name, ~
      ggplot(.x, aes(y = month_abb, x = value, fill = after_stat(x))) +
        geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
        scale_fill_viridis_c(option = "C") +   # remove alpha for stronger color
        theme_minimal() +
        theme(legend.position = "right") +     # turn on to confirm colors
        labs(x = .y, y = "month", fill = .y)
    )
  )
